...
static int_t xgzp_func(int argc, char **argv);
...
"          XGZP (cmd) (cmd) pressure sensor test\n";
...
static int_t xgzp_func(int argc, char **argv)
{
	...
	i2c_hw_xgzp_test(&pas, &temp);

	printf("i2c hardware xgzp test: pressure=%d Pascal, temperature=%d.%02d degree\n", pas, (int8_t)temp, abs((temp-(int8_t)temp)*100));
