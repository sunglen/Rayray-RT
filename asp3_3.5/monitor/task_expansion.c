...
ref_tst(ID tskid, T_RTST *pk_rtst)
{
	...
	if (TSTAT_RUNNABLE(tstat)) {
		...
		else if ((tstat & TS_WAITING_MASK) == TS_WAITING_SLP) {
			pk_rtst->tskwait = TTW_SLP;
		}
		else if ((tstat & TS_WAITING_MASK) == TS_WAITING_RDTQ) {
...
{
#if !defined(TOPPERS_ARM_M) || defined(TOPPERS_STM32F4_DISCOVERY) || defined(TOPPERS_STM32F4_YSPRO) || defined(TOPPERS_STM32F4_IMT407G)
